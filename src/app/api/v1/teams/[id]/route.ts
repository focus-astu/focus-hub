...
import { editTeam, deleteTeam } from "@/core/shared/infrastructure/config/dependencies"
...
export const PUT = async (request: NextRequest, { params }: RouteParams) => {
  try {
    ...
    console.error("PUT /api/v1/teams/[id] error:", msg)
...
export const DELETE = async (_request: Request, { params }: RouteParams) => {
  try {
    ...
    const msg = error instanceof Error ? error.message : "Internal server error"
    console.error("DELETE /api/v1/teams/[id] error:", msg)
    return NextResponse.json({ error: "Internal server error" }, { status: 500 })
  }
}
