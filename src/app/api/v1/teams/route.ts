...
import { createTeam, getTeams } from "@/core/shared/infrastructure/config/dependencies"
...
export const GET = async () => {
  try {
    ...
    console.error("GET /api/v1/teams error:", msg)
...
export const POST = async (request: NextRequest) => {
  try {
    ...
    if (!canManage) {
      return NextResponse.json({ error: "Only General Leaders or admins can manage teams" }, { status: 403 })
    }
    ...
    const msg = error instanceof Error ? error.message : "Internal server error"
    console.error("POST /api/v1/teams error:", msg)
    return NextResponse.json({ error: "Internal server error" }, { status: 500 })
  }
}
