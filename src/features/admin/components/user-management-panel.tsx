...
import { Users, Search, Settings2, Shield } from "lucide-react"
        ...
        {filteredUsers.map((user) => {
          ...
          return (
            ...
            >
              <div className="flex items-start justify-between gap-3">
                <div className="min-w-0 flex-1">
                  <div className="flex items-center gap-2">
                    ...
                    {user.isSuper && (
                      <span className="inline-flex shrink-0 items-center gap-1 rounded-full bg-gradient-to-r from-amber-100 to-amber-50 px-2 py-0.5 text-[10px] font-bold text-amber-800">
                        <Shield className="h-2.5 w-2.5" />
                        SUPER
                      </span>
                    )}
                ...
                <button
                  type="button"
                  onClick={() => setRoleTarget(user)}
                  className="inline-flex shrink-0 items-center gap-1.5 rounded-lg bg-slate-100 px-3 py-1.5 text-xs font-semibold text-slate-700 transition-all hover:bg-slate-200"
                  tabIndex={0}
                  aria-label={`Manage roles for ${user.name}`}
                >
                  <Settings2 className="h-3.5 w-3.5" />
              ...
              <p className="mt-2 text-xs text-slate-400">Joined {formatDate(user.createdAt)}</p>
            </div>
          )
        })}
